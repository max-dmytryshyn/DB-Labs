...
SELECT mc.name, mc.surname, mc.birth_date, h.name AS hospital_name, h.address AS hospital_address, 
	dpf.name AS doctor_name, dpf.surname AS doctor_surname,mc.tracker_date, mc.systolic_blood_pressure,
    mc.diastolic_blood_pressure, mc.heart_rate, mc.temperature, mc.doctor_appointment_date, mc.recommendation,
    mc.drugs
    FROM patient
...
INNER JOIN 
    ...
    td.diastolic_blood_pressure, td.heart_rate, td.temperature, da.date AS doctor_appointment_date, da.recommendation,
    da.drugs
    FROM medical_card AS mc
    INNER JOIN tracker_data AS td ON mc.id = td.medical_card_id
    INNER JOIN
    (SELECT da.date, da.recommendation, da.medical_card_id, dahd.drugs FROM doctor_appointment AS da
		INNER JOIN(
